...
library StorageSlot {
    ...
    /// @notice Maximum bytes of data that can be stored inline in a single slot.
    uint256 internal constant MAX_CAPACITY = 0x1c;

    /// @notice Mask for extracting the length value from storage.
    uint256 internal constant LENGTH_MASK = 0xffffffff;

    /// @notice Size of the length field in bytes.
    uint256 internal constant LENGTH_SIZE = 0x04;

    /// @notice Bit shift amount to position length in the high-order bytes.
    uint256 internal constant LENGTH_SHIFT = 0xe0;

    /// @notice Derive an ERC-7201 slot from a string.
    function compute(string memory namespace) internal pure returns (bytes32 slot) {
    ...
    /// @notice Casts a {AddressSlot} slot back to its underlying bytes32 value.
    function toBytes32(AddressSlot slot) internal pure returns (bytes32) {
        return AddressSlot.unwrap(slot);
    }

    /// @notice Returns the value held at location `slot` in persistent storage.
    ...
    /// @notice Returns the value held at location `slot` in transient storage.
    ...
    /// @notice Returns whether the transient storage at `slot` is empty or not.
    ...
    /// @notice Casts a {BooleanSlot} slot back to its underlying bytes32 value.
    function toBytes32(BooleanSlot slot) internal pure returns (bytes32) {
    ...
    /// @notice Casts a {Bytes32Slot} slot back to its underlying bytes32 value.
    function toBytes32(Bytes32Slot slot) internal pure returns (bytes32) {
    ...
    /// @notice Casts a {Uint256Slot} slot back to its underlying bytes32 value.
    function toBytes32(Uint256Slot slot) internal pure returns (bytes32) {
    ...
    /// @notice Casts a {Int256Slot} slot back to its underlying bytes32 value.
    function toBytes32(Int256Slot slot) internal pure returns (bytes32) {
    ...
    function asBytes(bytes32 slot) internal pure returns (BytesSlot) {
        return BytesSlot.wrap(slot);
    }

    /// @notice Casts a {BytesSlot} slot back to its underlying bytes32 value.
    function toBytes32(BytesSlot slot) internal pure returns (bytes32) {
    ...
    function tload(BytesSlot slot) internal view returns (bytes memory result) {
        assembly ("memory-safe") {
            result := mload(0x40)
            mstore(result, 0x00)
            mstore(add(result, MAX_CAPACITY), tload(slot))
    ...
    function asString(bytes32 slot) internal pure returns (StringSlot) {
        return StringSlot.wrap(slot);
    }

    /// @notice Casts a {StringSlot} slot back to its underlying bytes32 value.
    function toBytes32(StringSlot slot) internal pure returns (bytes32) {
